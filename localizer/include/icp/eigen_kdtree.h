...
class TreeNode_ {
 public:
  ...
  PointType* bestMatchFast(const PointType& query, Scalar norm) {
    ...
    //std::cerr << "___ called best match fast: \n" << std::endl;
    //if(_left ) std::cerr << "mean left: \n" << _left ->_mean << std::endl;
    //if(_right ) std::cerr << "mean rigth: \n" << _right ->_mean << std::endl;

    if (!_left && !_right) {

      //std::cerr << "___ in a leaf\n";
      return bruteForceBestMatch(_begin, _end, query, norm);
    }
    //std::cerr << "..calculate distance: ";
    Scalar distance_from_split_plane = (query - _mean).dot(_normal);
    //std::cerr << distance_from_split_plane << std::endl;

    if (distance_from_split_plane < Scalar(0)){
      //std::cerr << "__ going left\n";
      return _left->bestMatchFast(query, norm);
    }
    else{
      //std::cerr << "__ going right\n";
  ...
  PointType* bestMatchFull(const PointType& query, Scalar norm) {
    //std::cerr << "___ called best match full: \n" << std::endl;
